...
각 대학별 상세 분석 (대학당 3~4줄 이내로 간결하게):
- 합격 조건, 현재 갭, 핵심 전략을 대학별로 간단히 서술
...
|------|------|
| 현재 수준 | 상/중/하 |
| Before (현재 원문) | "(현재 기록된 세특 내용 전문)" |
| 문제점 분석 | 1) ... 2) ... 3) ... |
| After (개선안) | "(합격자 수준으로 직접 재작성한 세특 전문 — 탐구 동기, 과정, 어려움 극복, 결과, 발전 방향이 모두 포함)" |
| 개선 포인트 | 1) 탐구 계기 추가 2) 구체적 과정 서술 3) 전공 연계 강화 4) 성장 서사 포함 |

위 표를 주요 교과(최소 3개 이상)에 대해 각각 작성하라.

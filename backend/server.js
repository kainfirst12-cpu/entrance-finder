...
## 한눈에 보기
(핵심 3~5줄: 이 학교의 성격과 결론)
## 성취도 해석 — 국어·영어·수학
(학년·학기별 A 비율/평균 추이, 과목 간 균형, 시험 난이도·상위권 두께 추정. 표를 하나 넣어도 좋음)
## 학교 규모와 등급 구조
(재적·1등급 자리·학급 수·선택과목 개설 폭이 내신에 미치는 영향)
## 이런 학생에게 유리합니다
(학생 유형별로 3~5개, 각 항목에 근거 수치)
## 이런 학생에게 불리합니다
(학생 유형별로 3~5개, 각 항목에 근거 수치)
## 입시 전략 제안
(이 학교에 다닌다면 학생부교과·학생부종합·정시 중 어느 쪽에 무게를 두어야 하는지, 과목 선택·시기별 포인트)
## 유의사항
(자료의 한계와 확인해야 할 것)`;
...
### 과목 개설과 선택 폭
### 학력 수준과 학습 분위기
(각 항목마다 어느 학교가 어떤 학생에게 유리한지 수치로 비교)
## 학생 유형별 추천
(예: 최상위권(의약학·SKY 교과전형 목표) / 중상위권 내신 확보 / 학종 활동 중심 / 정시·수능 중심 / 학습 분위기가 필요한 학생 — 유형마다 추천 학교와 이유)
## 학교별 유리·불리 정리
(학교마다 유리한 학생·불리한 학생을 2~3줄씩)
## 유의사항`;
...
[합격 사례 매칭(0단계)은 특히 꼼꼼히 보라]
이 섹션은 원장이 학부모 앞에 놓는 표라, 여기가 틀리면 상담 전체가 어긋난다. 아래를 어기면 "error"로 잡아라.
- 학생 내신보다 한참 위인 사례가 TOP에 올라와 있는가. 아래 격차 기준을 넘으면 그 사례는 올라오면 안 된다.
  · 학생부교과 사례: 학생이 0.5등급 초과로 낮으면 참고 제한, 1.0등급 초과면 참고 불가
  · 학생부종합 사례: 학생이 1.0등급 초과로 낮으면 참고 제한, 1.5등급 초과면 참고 불가
- 표에 합격자 내신 · 학생 내신 · 격차가 드러나 있는가. 하나라도 빠졌으면 지적하라.
- 활동이 닮았다는 이유만으로 상위권 사례를 붙이지 않았는가. 활동 유사성은 내신 격차를 대체하지 못한다.
- 자사고·특목고 사례를 일반고 학생에게 붙이며 고교 유형 차이를 적지 않았는가.
- 학생 내신이 미입력인데 성적 비교를 지어내지 않았는가.
⚠ 반대로, 이미 적혀 있는 격차·유사도·주의 문구를 "면책성 표현이니 빼라"고 지적하지는 마라.
  그건 면책이 아니라 사실이고, 지우면 표가 "여기 지원해도 된다"로 읽힌다.
지적할 때 section 은 "합격자 사례 매칭 분석" 으로 적어라.

[학생 정보]
이름: ${studentName}
학교: ${studentData?.school || '미입력'}
계열: ${studentData?.major || '미입력'}
내신: ${studentData?.gpa || '미입력'}등급   ← 사례 매칭의 격차는 이 값을 기준으로 따져라
...
- (나)를 인용할 때는 대괄호 안의 자료 제목을 함께 밝히십시오. 제목에 학년도가 들어 있으면
  그 학년도를 반드시 적으십시오. 질문의 학년도와 자료의 학년도가 다르면 그 사실을
  먼저 경고하십시오 — 학년도가 다른 전형계획을 그대로 적용하면 상담이 어긋납니다.`;
...
- search_knowledge 결과를 인용할 때는 자료의 제목을 함께 밝히십시오. 제목에 학년도가 있으면
  그 학년도를 적고, 질문의 학년도와 다르면 먼저 경고하십시오. 학년도가 다른 전형계획을
  그대로 적용하면 상담이 어긋납니다.
